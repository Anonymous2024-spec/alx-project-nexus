  ...
  return (
    <View className={getContainerClasses()}>
      <View className={getSpinnerWrapperClasses()}>
        <ActivityIndicator size={spinnerSizes[size]} color={colors[color]} />
      </View>

      {text && <Text className={getTextClasses()}>{text}</Text>}
    </View>
  );
};
...
export const LoadingStates = {
  ...
  Overlay: ({ text = "Loading..." }: { text?: string }) => (
    <LoadingSpinner size="medium" color="primary" text={text} overlay />
  ),

  // Inline loading (within components)
  Inline: ({ text }: { text?: string }) => (
    <LoadingSpinner size="small" color="primary" text={text} />
  ),
